...
The plugin does not implement image generation itself. Each plugin skill dispatches to a headless `codex exec` turn and lets Codex's `imagegen` skill drive the built-in `image_gen` tool, use attached reference/edit images, save the final artifact, and print a `SAVED: <path>` line for each output.

## Requirements

- Claude Code with plugin support.
- `@openai/codex` CLI v0.142.0 or later (v0.144+ recommended — it moves image generation to the extension-backed tool the instruction prefixes are written for). v0.153+ is supported: the headless auto-approval flag is detected at dispatch time.
