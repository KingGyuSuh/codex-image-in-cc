...
The ladder pick is also best-effort at spawn time, not just at probe time. The catalog can list a model the actual backend refuses — the canonical case is a custom/API-key `model_provider` that does not serve official slugs while the bundled catalog remains visible. Such a rejection kills the `codex exec` turn at startup (verified on 0.144.5: HTTP 400 `"The 'X' model is not supported…"` on stderr, empty stdout, no image tokens spent). When a **ladder-selected** orchestrator fails that way — non-zero exit plus a model/effort-rejection signature on stderr (`isOrchestratorRejection`) — the wrapper retries once with no `-m`/`-c`, landing on the codex config default that worked before the ladder existed. Unrelated failures (quota, auth, transport) do not match the signature and are not retried, so a real image-turn failure is never double-billed.
