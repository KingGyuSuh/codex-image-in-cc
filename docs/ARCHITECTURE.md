...
The ladder pick is also best-effort at spawn time, not just at probe time. The catalog can list a model the actual backend refuses — the canonical case is a custom/API-key `model_provider` that does not serve official slugs while the bundled catalog remains visible. Such a rejection kills the `codex exec` turn at startup (verified on 0.144.5 and 0.154.0: HTTP 400 `"The 'X' model is not supported…"` on stderr, empty stdout, no image tokens spent). When a **ladder-selected** orchestrator fails that way — non-zero exit, a model/effort-rejection signature in codex's `ERROR:` record (`isOrchestratorRejection`), and no `codex` / `exec` event line before it (`turnHadStarted`) — the wrapper retries once with no `-m`/`-c`, landing on the codex config default that worked before the ladder existed. Unrelated failures (quota, auth, transport) do not match the signature and are not retried, so a real image-turn failure is never double-billed. The classification (`shouldRetryWithoutOrchestrator`) looks only at codex's own stderr: `codex exec` echoes the full instruction under a `user` heading and later prints agent messages and tool output there too, so the wrapper first cuts the echoed instruction off (`codexOwnStderr` — it knows the exact text it sent, and if the bounded tail starts inside the echo it cuts at the instruction's last line), strips ANSI styling (codex colours stderr under `FORCE_COLOR` even when piped), and then treats everything from the first `ERROR:` line to the end as the error record (a pretty-printed JSON rejection from a custom provider spans several lines). A prompt that mentions "unknown model" or contains a standalone `codex` line therefore neither causes nor suppresses the fallback, and once a real `codex` or `exec` event line has appeared the turn got past startup and the image tool may already have been billed, so nothing is retried after that regardless of the error text.
