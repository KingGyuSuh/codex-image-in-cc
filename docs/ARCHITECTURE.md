...
The ladder pick is also best-effort at spawn time, not just at probe time. The catalog can list a model the actual backend refuses — the canonical case is a custom/API-key `model_provider` that does not serve official slugs while the bundled catalog remains visible. Such a rejection kills the `codex exec` turn at startup (verified on 0.144.5 and 0.154.0: HTTP 400 `"The 'X' model is not supported…"` on stderr, empty stdout, no image tokens spent). When a **ladder-selected** orchestrator fails that way — non-zero exit, a model/effort-rejection signature on one of codex's `ERROR:` stderr lines (`isOrchestratorRejection`), and no `codex` / `exec` event line in the captured stderr tail (`turnHadStarted`) — the wrapper retries once with no `-m`/`-c`, landing on the codex config default that worked before the ladder existed. Unrelated failures (quota, auth, transport) do not match the signature and are not retried, so a real image-turn failure is never double-billed. Two details keep that promise honest: `codex exec` also echoes the user prompt, agent messages, and tool output to stderr, so only `ERROR:` lines are classified (a prompt that mentions "unknown model" must not turn a transport error into a retry); and once a bare `codex` or `exec` event line has appeared the turn got past startup and the image tool may already have been billed, so nothing is retried after that regardless of the error text.
